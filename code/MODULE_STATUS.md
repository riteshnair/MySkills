...
| SafeTensors inspection | PASS | Valid descriptor, dtype/shape byte-size, file-bound, and overlap rejection fixtures | Source dtype is preserved; explicit SafeTensors quantize-cache conversion is not implemented |
...
| CPU MoE routing oracle | PASS | Deterministic top-k selection, tie-breaking, all-softmax/selected-only weighting, normalization, selected-output combination, NaN, and bound tests | Expert tensor loading, routed expert MLP execution, capacity policies, and model-specific router semantics remain pending |
| Lazy/disk-backed residency | IN PROGRESS | Model handles bind validated data bases to range-checked relative tensor spans; on-demand reads are tested | Add mmap/window adapters, descriptor-name/index API, prefetch, and residency traces |
