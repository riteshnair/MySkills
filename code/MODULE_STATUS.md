...
| GGUF inspection | PASS | v3 header, bounded metadata, alignment, tensor descriptor, dimension, type, offset, expert-count, and experts-per-token tests | Tensor byte-size validation for every quantized block type and expert-name/index mapping are still pending |
