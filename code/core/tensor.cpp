...
lm_status lm_tensor_validate(const lm_tensor *tensor) {
    ...
    if (tensor->quant_format != LM_QUANT_BLOCK_STORAGE || tensor->quant_elements_per_block == 0u ||
        tensor->quant_bytes_per_block == 0u || elements % tensor->quant_elements_per_block != 0u)
